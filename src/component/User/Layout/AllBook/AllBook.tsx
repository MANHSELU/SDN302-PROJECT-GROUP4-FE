...
export default function AllBooks() {
    ...
    const [selectedCategory, setSelectedCategory] = useState<string>(""); // 🟡 Thêm state này
    ...
    useEffect(() => {
        fetch(APIBook.getCategorylimit)
            ...
            .catch((err) =>
                console.error("❌ Lỗi khi gọi danh sách category:", err)
            );
    }, []);
    ...
    useEffect(() => {
        ...
        const url = `${APIBook.getBook}?page=${currentPage}${search ? `&keyword=${encodeURIComponent(search)}` : ""
            }${selectedCategory ? `&categoryTitle=${encodeURIComponent(selectedCategory)}` : ""}`;
    ...
    }, [currentPage, search, selectedCategory]); // 🟡 thêm selectedCategory
        ...
        <div className="min-h-screen bg-slate-900 text-white p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-8">
                ...
                {listCategory.map((cat) => (
                    <button
                        key={cat._id}
                        onClick={() => setSelectedCategory(cat.title)} // 🟡 Cập nhật category đang chọn
